...
class Outgoing_stok_atk extends Admin_Controller
{
  ...
  public function request($id = null)
  {
    if ($this->input->post()) {
      ...
      if (!empty($data['detail'])) {
        foreach ($detail as $val => $valx) {
          ...
          if ($qty_packing > 0) {
            ...
            $qty_stock = 0;
            $get_qty_stock = $this->db->get_where('warehouse_stock', ['id_material' => $valx['id'], 'id_gudang' => 1])->row();
      ...
      if ($this->db->trans_status() === FALSE || $valid_qty_stock < 1) {
        ...
        move_warehouse_stok($ArrStock, 1, $id_costcenter, $kode_trans, null);
  ...
  {
    $this->outgoing_stok_atk_model->get_data_outgoing_atk();
  }
}
