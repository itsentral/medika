...
class Consumable_model extends BF_Model
{
	...
	public function get_json_accessories()
	{
		...
		foreach ($query->result_array() as $row) {
			...
			$get_user = $this->db->get_where('users', ['id_user' => $last_create])->row();

			$nm_user = (!empty($get_user)) ? $get_user->nm_lengkap : '';

			$nestedData[]	= "<div align='left'>" . strtolower($nm_user) . "</div>";
	...
	public function get_json_unit()
	{
		...
		foreach ($query->result_array() as $row) {
			...
			$this->db->where('a.id_user', $last_create);
	...
	public function getAccessorieswithUnit()
	{
		$this->db->select('a.*, b.code as nm_satuan, c.code as nm_pack');
		...
		$get_accessories = $this->db->get()->result();
