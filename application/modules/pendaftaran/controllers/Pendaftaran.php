...
class Pendaftaran extends Admin_Controller {
	...
	function get_nobed(){
        ...
        $datnobed = $this->Pendaftaran_model->pilih_combobox_array('rs_bed_nomor', $where)->result();
        // print_r($datnobed);
        // exit();
	...
	public function savePendaftaranRi(){
		...
		$code = $this->Pendaftaran_model->generate_no_rm();
		$no_tr = $this->Pendaftaran_model->generate_no_tr_ri();
		$no_reg = $this->Pendaftaran_model->generate_no_reg_ri();
		$antrian = $this->Pendaftaran_model->cek_antrian($dokter,$jam,$tgl_daftar);
		$this->db->trans_begin();
		...
		$this->Pendaftaran_model->getUpdate('rs_bed_nomor',$data_update,$where);
	...
	public function TrPendaftaranRj($tr){
		...
		$this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
		$data = $this->Pendaftaran_model->GetPasienRJ($tr);
		$getlayanan = $this->Pendaftaran_model->pilih_grouplayanan('layanan',$session['id_div'])->result();
	    $getdokter  = $this->Pendaftaran_model->pilih_dokter()->result();
		$this->template->page_icon('fa fa-edit');
	...
	public function TrPendaftaranLab($tr){
		...
		$this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
		$data = $this->Pendaftaran_model->GetPasienLab($tr);
		$getlayanan = $this->Pendaftaran_model->pilih_grouplayanan('layanan',$session['id_div'])->result();
	    $getdokter  = $this->Pendaftaran_model->pilih_dokter()->result();
		$this->template->page_icon('fa fa-edit');
	...
	public function saveBilling(){
		...
		$data = $this->Pendaftaran_model->GetTarif($grouplayanan); 
		foreach ($data as $d) {    
        ...
        $mark = $this->Pendaftaran_model->GetMarkup($jaminan,$kodecoa);
	...
	public function load_billing()
    {
        ...
        $numb = 1;
        $data = $this->Pendaftaran_model->GetTransaksiDt($noreg,$divisi);   
	...
	public function hapusDetail(){
		...
		$this->Pendaftaran_model->DeleteDetail($id);
	...
	public function Update_PasienLama($medrec){
		...
		$datdept    = $this->Pendaftaran_model->pilih_departemen()->result();
		$datcust    = $this->Pendaftaran_model->pilih_customer()->result();
		$datkota    = $this->Pendaftaran_model->pilih_kota('')->result();
		$datkec     = $this->Pendaftaran_model->pilih_kec('')->result();
		$datkel     = $this->Pendaftaran_model->pilih_kel('')->result(); 
	...
	public function Update_Tr_Rj(){
        ...
        $session = $this->session->userdata('app_session');
		$data = $this->Pendaftaran_model->GetPasienLama($medrec);
		$data2 = $this->Pendaftaran_model->GetPasienRJ($noreg);
		$this->template->page_icon('fa fa-edit');
		$prov		= $this->Pendaftaran_model->get_data('provinsi');
		$kota 		= $this->Pendaftaran_model->get_data('kota');
		$datprov    = $this->Pendaftaran_model->pilih_provinsi()->result();
        ...
        $datdok  = $this->Pendaftaran_model->pilih_dokter()->result();
		
        $datjad = $this->Pendaftaran_model->get_jadwal()->result();
        
        $datjam = $this->Pendaftaran_model->get_jadwal()->result();
		
		$datjaminan = $this->Pendaftaran_model->get_jaminan()->result();
	...
	public function TrPendaftaranRi($tr){
		...
		$this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
		$data = $this->Pendaftaran_model->GetPasienRI($tr);
		$getlayanan = $this->Pendaftaran_model->pilih_grouplayanan('layanan',$session['id_div'])->result();
	    $getdokter  = $this->Pendaftaran_model->pilih_dokter()->result();
		$this->template->page_icon('fa fa-edit');
	...
	public function load_billingRi()
    {
        ...
        $numb = 1;
        $data = $this->Pendaftaran_model->GetTransaksiDtRi($noreg,$divisi);   
	...
	public function hapusDetailRi(){
		...
		$this->Pendaftaran_model->DeleteDetailRi($id);
	...
	public function addPendaftaranlab(){
		$this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session'); 
		$this->template->page_icon('fa fa-pencil');
		$prov = $this->Pendaftaran_model->get_data('provinsi');
		$kota = $this->Pendaftaran_model->get_data('kota');
		$datprov    = $this->Pendaftaran_model->pilih_provinsi()->result();
		$datdept    = $this->Pendaftaran_model->pilih_departemenlab()->result();
		$datcust    = $this->Pendaftaran_model->pilih_customer()->result();
		$labkategori   = $this->Pendaftaran_model->get_data('rs_kategorilab');
		
		 $this->template->set('labkategori',$labkategori);
	...
	public function list_pasienlab()
      {  
	    $this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
	    $data = $this->Pendaftaran_model->GetTransaksiLab();
        $this->template->set('results', $data);
...
public function Update_Tr_Lab(){
        ...
        $session = $this->session->userdata('app_session');
		$data = $this->Pendaftaran_model->GetPasienLama($medrec);
		$data2 = $this->Pendaftaran_model->GetPasienLab($noreg);
		$this->template->page_icon('fa fa-edit');
		$prov		= $this->Pendaftaran_model->get_data('provinsi');
		$kota 		= $this->Pendaftaran_model->get_data('kota');
		$datprov    = $this->Pendaftaran_model->get_data('provinsi');
		$datdept    = 0;
		$datcust    = 0;
		$datkota    = 0;
		$datkec     = 0;
	...
	// FORM DAFTAR PASIEN LAMA
	//------------------------------------------------------------
	public function Daftar_PasienLab($medrec){
        ...
        $this->template->set('results', $data);
		$this->template->title('Pendaftaran laboratorium');
		$this->template->render('form_add_pendaftaran_lab_lama');
	...
	public function pasien_lama()
      {  
		...
		$this->template->set('search', $this->uri->segment(3));
        $this->template->title('Pendaftaran Laboratorium');
        $this->template->render('list_ssp');
      }
	  
	 public function server_side_inv()
	{
		$this->Pendaftaran_model->get_data_json_inv();
	}
	
	 	
	  public function cetak_barcode3($tr)
    {
        $pasien = $this->Pendaftaran_model->GetPasienLab($tr);
		
		// print_r($pasien->noreg);
		// exit;
        ...
        $barcode = $this->barcodegeneratorci->generate($pasien->noreg);

        // Simpan barcode ke file PNG
        $barcode_path = FCPATH . 'uploads/barcode_' . $pasien->noreg . '.png';
        file_put_contents($barcode_path, $barcode);

        $data = [
            'pasien' => $pasien->noreg,
            'barcode_image' => base_url('uploads/barcode_' . $pasien->noreg . '.png')
        ];

        $this->template->set('data',$data);
		$this->template->title('Cetak Barcode');
        $this->template->render('cetak_barcode'); 
    }
	

   public function cetak_barcode2($tr)
{
    $this->load->library('BarcodeGeneratorCI');
	
	$pasien = $this->Pendaftaran_model->GetPasienLab($tr);
	$no_pendaftaran =$pasien->noreg;
	
    $barcode = $this->barcodegeneratorci->generate($no_pendaftaran);
	
	// print_r($no_pendaftaran);
	// print_r(base64_encode($barcode));
	// exit;

    $data['no_pendaftaran'] = $no_pendaftaran;
	$data['tgl_daftar']     = date('d-m-Y', strtotime($pasien->tgl_registrasi));
	$data['nama'] = $pasien->nama_pasien.$pasien->status_pasien;
	$data['lahir'] = date('d-m-Y', strtotime($pasien->tgl_lahir));
