<div class="box box-primary">
    ...
    <?= form_open($this->uri->uri_string(),array('id'=>'frm_users','name'=>'frm_users','role'=>'form','class'=>'form-horizontal')) ?>
    	<div class="box-body">
			...
			<div class="form-group <?= form_error('divisi') ? ' has-error' : ''; ?>">
				 <label for="divisi" class="col-md-2 control-label"><?= lang('divisi') ?></label>
					 <div class="col-md-3">
					  <select id="divisi" name="divisi" class="form-control input-sm select2"  required onchange="getdept()">
                            <option value=""></option>
                            <?php foreach ($datdiv as $key => $st) : ?>
                            <option value="<?= $st->id_div; ?>" <?= set_select('divisi', $st->id_div, isset($data->nama_div) && $data->nama_div == $st->id_div) ?>>
                            <?= strtoupper($st->nama_div); ?>
                            </option>
                            <?php endforeach; ?>
                      </select>
					 </div>
			...
			<div class="form-group <?= form_error('departemen') ? ' has-error' : ''; ?>">
				 <label for="departemen" class="col-md-2 control-label"><?= lang('departemen') ?></label>
					 <div class="col-md-3">
					  <select id="departemen" name="departemen" class="form-control input-sm select2" onchange="get_dokter()">
                            <option value=""></option>
			...
			<div class="form-group <?= form_error('dokter') ? ' has-error' : ''; ?>">
					<label for="dokter" class="col-md-2 control-label"><?= lang('dokter') ?></label>
			     	 <div class="col-md-3">
					 <select id="dokter" name="dokter" class="form-control input-sm select2" >
...
$(function() {
	$('.select2').select2();
       ...
       function get_dokter(){
        ...
        $.ajax({
            type:"GET",
            url:siteurl+"users/setting/get_dokter",
            data:"dept="+dept,
