 ...
 <div class="box box-primary">
    <div class="box-body">
		<form id="data-form" method="post">
						   ...
						   <div class="input_fields_wrap2">
									    ...
									    <label for="customer">Inventory Name</label>
										 ...
										 <div class="col-md-6" hidden>
											<input type="text" class="form-control" id="" required name="hd1[1][id_inventory]" value="<?= $bentuk->id_bentuk ?>" placeholder="Nama Inventory">
									    </div>
									    <div class="col-md-6">
											<input type="text" class="form-control" id="" required name="hd1[1][nm_inventory]" value="<?= $bentuk->nm_bentuk ?>" placeholder="Nama Inventory">
									    </div>
										</div>
									...
									<table class='table table-bordered table-striped'>
										<thead>
											<tr class='bg-blue'>
												...
												<td align='center' hidden><b>Action</b></td>	
											</tr>
					...
					<input type="text" class="form-control input-sm" name="data2['<?= $numb ?>'][id_dimensi]" value="<?= $dimensi->id_dimensi ?>" id="data2_'<?= $numb ?>'_id_dimensi" label="FALSE" div="FALSE">
												</td>
												<td align="left">
					<input type="text" class="form-control input-sm" readonly name="data2['<?= $numb ?>'][nm_dimensi]" value="<?= $dimensi->nm_dimensi ?>"  id="data2_'<?= $numb ?>'_nm_dimensi" label="FALSE" div="FALSE">
												</td>
												<td align="center" hidden>
					<a class="btn btn-danger btn-sm delete" href="javascript:void(0)" title="Delete" data-id_dimensi="<?=$dimensi->id_dimensi?>"><i class="fa fa-trash"></i>
				</a>
												</td>
											</tr>
											<?php
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#add-payment').click(function(){
			...
			Template	+='<td align="left">';
					Template	+='<input type="text" class="form-control input-sm" name="data1['+loop+'][nm_dimensi]" id="data1_'+loop+'_nm_dimensi" label="FALSE" div="FALSE">';
			Template	+='</td>';
	...
	$(document).on('click', '.delete', function(e){
		...
		function(){
		  $.ajax({
			  type:'POST',
			  url:siteurl+'master_bentuk/delDetail',
			  dataType : "json",
			  data:{'id':id},
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'master_bentuk/saveEditbentuk';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
