...
class Master_customer extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Master_Customer/Master_customer_model',
    ...
    public function index()
    {
       $this->auth->restrict($this->viewPermission);
        ...
        $data = $this->Master_customer_model->find_all_by(array('deleted' => '0'));
        $this->template->set('results', $data);
        $this->template->title('Customer');
        $this->template->render('view_master_customer');
    }
	...
	public function add_customer(){
		...
		$prov = $this->Master_customer_model->get_data('provinsi');
		$kota = $this->Master_customer_model->get_data('kota');
		$data = [
			'prov' => $prov,
			'kota' => $kota
		];
	...
	public function saveCustomer(){
		...
		$code 		= $this->Master_customer_model->generate_id();
		$this->db->trans_begin();
		$data = [
			'id_customer'		=> $code,
			...
			'note'				=> $post['note'],
			'aktif'				=> $post['status'],
			'gelar_mkt'			=> $post['gelar_mkt'],
			'marketing'			=> $post['marketing'],
			'no_ktp'			=> $post['idnumber'],
			'alamat_ktp'		=> $post['idaddress'],
			'emailmkt'			=> $post['emailmkt'],
			'telpmkt'			=> $post['telpmkt'],
