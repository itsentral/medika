<?php
$ENABLE_ADD     = has_permission('Pemeriksaan.Add');
$ENABLE_MANAGE  = has_permission('Pemeriksaan.Manage');
$ENABLE_VIEW    = has_permission('Pemeriksaan.View');
$ENABLE_DELETE  = has_permission('Pemeriksaan.Delete');
?>
<style type="text/css">
    thead input {
        width: 100%;
...
<div class="modal modal-default fade" id="dialog-popup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <script type="text/javascript">
        ...
        $(document).on('submit', '#frm-data', function(e) {
            ...
            swal({
                ...
                if (next) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: siteurl + active_controller + 'save_pemeriksaan',
                        data: frmdata,
                        cache: false,
                        dataType: 'json',
        ...
        $(document).on('click', '.del', function() {
            ...
            swal({
                ...
                if (next) {
                    $.ajax({
                        type: 'post',
                        url: siteurl + active_controller + 'delete_pemeriksaan',
                        data: {
                            'id': id
                        },
                        cache: false,
        ...
        function add() {
            $.ajax({
                type: 'post',
                url: siteurl + active_controller + 'add_pemeriksaan',
                cache: false,
                success: function(result) {
                    $('#ModalView').html(result);
                    $('#dialog-popup').modal('show');
        ...
        function DataTables() {
            var DataTables = $('#example1').dataTable({
                ...
                ajax: {
                    type: 'post',
                    url: siteurl + active_controller + 'get_data_pemeriksaan',
                    cache: false,
                    dataType: 'json',
