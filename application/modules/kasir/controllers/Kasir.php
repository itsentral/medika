...
class Kasir extends Admin_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('Kasir/Kasir_model',
    ...
    public function index()
    {
        ...
        //$data = $this->Kasir_model->rekap_data($session['kdcab'])->result();
        $data = $this->Kasir_model->find_all_by();
        $this->template->set('results', $data);
        $this->template->title('Kasir');
        $this->template->render('index');
		
    }
	
	public function addKasirRj(){
		$this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-pencil');
		$prov = $this->Kasir_model->get_data('provinsi');
		$kota = $this->Kasir_model->get_data('kota');
		$datprov    = $this->Kasir_model->pilih_provinsi()->result();
		$datdept    = $this->Kasir_model->pilih_departemen()->result();
		$datcust    = $this->Kasir_model->pilih_customer()->result();
		
		$this->template->set('datprov',$datprov);
		$this->template->set('datdept',$datdept);
		$this->template->set('datcust',$datcust);
        
        $this->template->title('Kasir Pasien Rawat Jalan ');
	...
	function get_kota(){
        ...
        $datkota = $this->Kasir_model->pilih_kota($provinsi)->result();
        // print_r($datkota);
        // exit();
	...
	function get_kecamatan(){
        $kota = $_GET['kota'];
        $datkec = $this->Kasir_model->pilih_kec($kota)->result();
         // print_r($datkec);
         // exit();
	...
	function get_kelurahan(){
        $kec = $_GET['kec'];
        $datkel = $this->Kasir_model->pilih_kel($kec)->result();
        // print_r($datkota);
        // exit();
	...
	function get_dokter(){
		...
		$hari = $_GET['hari'];
		$jam = $_GET['jam'];
        $datdokter = $this->Kasir_model->pilih_deptdokter($dept,$hari,$jam)->result();
         // print_r($dept, $hari);
         // exit();
	...
	function get_jaminan(){
        $cust = $_GET['cust'];
        $datcust = $this->Kasir_model->pilih_jaminan($cust)->result();
        // print_r($datcust);
        // exit();
	...
	function get_jadwal(){
        $dept = $_GET['dept'];
        $datcust = $this->Kasir_model->pilih_jadwal($dept)->result();
        // print_r($datcust);
        // exit();
	...
	function get_jam(){
        $dept = $_GET['dept'];
		$hari = $_GET['hari'];
        $datcust = $this->Kasir_model->pilih_jam($dept,$hari)->result();
        // print_r($datcust);
        // exit();
        echo "<select id='jadwal' name='jadwal' class='form-control input-sm select2'>";
        echo "<option value=''>-- Pilih Jadwal --</option>";
	...
	public function cek_dokter(){
        ...
        $cek = $this->Kasir_model->cek_dokter($rm,$dept,$hari,$jam,$dokter);
	...
	public function saveKasirRj(){
		...
		$code = $this->Kasir_model->generate_no_rm();
		$no_tr = $this->Kasir_model->generate_no_tr();
		$no_reg = $this->Kasir_model->generate_no_reg();
		$antrian = $this->Kasir_model->cek_antrian($dokter,$jam,$tgl_daftar);
		$this->db->trans_begin();
		$data = [
			'medrec'		    => $code,
	...
	public function search_pasien()
      {  
	    $this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
	    $data = $this->Kasir_model->GetPasien($this->uri->segment(3));
        $this->template->set('results', $data);
	...
	public function Daftar_PasienLama($medrec){
		...
		$this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
		$data = $this->Kasir_model->GetPasienLama($medrec);
	...
	// PROSES SAVE PENDAFTARAN RJ
	// -------------------------------------------------------------
	public function saveKasirRj_Lama(){
		$this->auth->restrict($this->addPermission);
		$post = $this->input->post();
		$birthdate = $this->input->post('tgl_lahir');
		$tgl_lhr = date("Y-m-d", strtotime($birthdate));
	...
	public function list_pasienrj()
      {  
	    $this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
	    $data = $this->Kasir_model->GetTransaksiRj();
        $this->template->set('results', $data);
	...
	public function rawat_inap()
    {
		...
		$this->template->page_icon('fa fa-users');
        $this->template->set('results', $data);
        $this->template->title('Kasir Pasien Rawat Inap');
	...
	public function Daftar_PasienRi($medrec){
		...
		$data = $this->Kasir_model->GetPasienLama($medrec);
		$this->template->page_icon('fa fa-edit');
		$kelas		= $this->Kasir_model->get_data('rs_kelas');
		$dokter		= $this->Kasir_model->get_data('rs_dokter');
		$kota 		= $this->Kasir_model->get_data('kota');
		$datprov    = $this->Kasir_model->pilih_provinsi()->result();
		$datdept    = $this->Kasir_model->pilih_departemen()->result();
		$datcust    = $this->Kasir_model->pilih_customer()->result();
	...
	function get_kelas(){
        ...
        $datkelas = $this->Kasir_model->pilih_ruang($kelas)->result();
        // print_r($datcust);
        // exit();
	...
	function get_bed(){
        ...
        $datkelas = $this->Kasir_model->pilih_combobox('rs_bed','id_ruang', $ruang)->result();
        // print_r($datcust);
        // exit();
	...
	function get_nobed(){
        ...
        $datnobed = $this->Kasir_model->pilih_combobox_array('rs_bed_nomor', $where)->result();
        // print_r($datnobed);
        // exit();
	...
	public function saveKasirRi(){
		...
		$code = $this->Kasir_model->generate_no_rm();
		$no_tr = $this->Kasir_model->generate_no_tr_ri();
		$no_reg = $this->Kasir_model->generate_no_reg_ri();
		$antrian = $this->Kasir_model->cek_antrian($dokter,$jam,$tgl_daftar);
		$this->db->trans_begin();
		...
		$this->Kasir_model->getUpdate('rs_bed_nomor',$data_update,$where);
		...
		if($this->db->trans_status() === FALSE){
			...
			$keterangan = 'SUKSES, Simpan Data Kasir RI';
			$status1 = 1;
			$nm_hak_akses = $this->addPermission;
			$kode_universal = 'Newdata';
			$jumlah = 1;
	...
	public function list_pasienri()
      {  
	    $this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
	    $data = $this->Kasir_model->GetTransaksiRi();
        $this->template->set('results', $data);
	...
	public function TrKasirRj($tr){
		...
		$this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
		$data = $this->Kasir_model->GetPasienRJ($tr);
		$getlayanan = $this->Kasir_model->pilih_grouplayanan('layanan',$session['id_div'])->result();
	    $getdokter  = $this->Kasir_model->pilih_dokter()->result();
		$this->template->page_icon('fa fa-edit');
	...
	public function saveBilling(){
		...
		$data = $this->Kasir_model->GetTarif($grouplayanan); 
		foreach ($data as $d) {    
        ...
        $mark = $this->Kasir_model->GetMarkup($jaminan,$kodecoa);
	...
	public function load_billing()
    {
        ...
        $numb = 1;
        $data = $this->Kasir_model->GetTransaksiDt($noreg,$divisi);   
	...
	public function hapusDetail(){
		...
		$this->Kasir_model->DeleteDetail($id);
	...
	public function Update_PasienLama($medrec){
		...
		$datdept    = $this->Kasir_model->pilih_departemen()->result();
		$datcust    = $this->Kasir_model->pilih_customer()->result();
		$datkota    = $this->Kasir_model->pilih_kota('')->result();
		$datkec     = $this->Kasir_model->pilih_kec('')->result();
		$datkel     = $this->Kasir_model->pilih_kel('')->result(); 
	...
	public function Update_Tr_Rj(){
        ...
        $session = $this->session->userdata('app_session');
		$data = $this->Kasir_model->GetPasienLama($medrec);
		$data2 = $this->Kasir_model->GetPasienRJ($noreg);
		$this->template->page_icon('fa fa-edit');
		$prov		= $this->Kasir_model->get_data('provinsi');
		$kota 		= $this->Kasir_model->get_data('kota');
		$datprov    = $this->Kasir_model->pilih_provinsi()->result();
	...
	public function TrKasirRi($tr){
		...
		$this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
		$data = $this->Kasir_model->GetPasienRI($tr);
		$getlayanan = $this->Kasir_model->pilih_grouplayanan('layanan',$session['id_div'])->result();
	    $getdokter  = $this->Kasir_model->pilih_dokter()->result();
		$this->template->page_icon('fa fa-edit');
	...
	public function load_billingRi()
    {
        ...
        $numb = 1;
        $data = $this->Kasir_model->GetTransaksiDtRi($noreg,$divisi);   
	...
	public function hapusDetailRi(){
		...
		$this->Kasir_model->DeleteDetailRi($id);
