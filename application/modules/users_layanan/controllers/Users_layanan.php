...
class Users_layanan extends Admin_Controller {
	...
	public function edit_userlayanan()
    {
        $this->auth->restrict($this->viewPermission);
        ...
        $this->template->title(lang('users_manage_title'));
        $this->template->set("numb", $offset+1);
        $this->template->render('edit_user_layanan');
    ...
    public function create()
    {
		...
		$datdept    = $this->users_model->pilih_departemen()->result();
		$datdiv    = $this->users_model->pilih_divisi()->result();
	...
	function get_dokter(){
        $dept = $_GET['dept'];
		$datdokter = $this->users_model->pilih_deptdokter($dept)->result();
         // print_r($dept);
         // exit();
        ...
        echo "<option value=''>--Pilih Dokter--</option>";
                foreach ($datdokter as $key => $st) :
				      echo "<option value='$st->id_dokter' set_select('model', $st->id_dokter, isset($data->id_dokter) && $data->id_dokter == $st->id_dokter)>$st->id_dokter|$st->nama_dokter
                    </option>";
                endforeach;
        echo "</select>";
    }
	...
	public function Tambah_Layanan(){	
		...
		$getlayanan = $this->Pendaftaran_model->pilih_grouplayanan('layanan','0')->result();
		...
		$this->template->set('nm_div', $nm_div);
		$this->template->set('id_user', $id_user);
		$this->template->set('id_div', $id_div);
		$this->template->set('jaminan', $id_div);
		$this->template->title('Tambah List Layanan');
        $this->template->render('tambah_user_layanan');
		
	}
	
	// PROSES SAVE USER LAYANAN
	// -------------------------------------------------------------
	public function save_userslayanan(){
		...
		$cek        = $this->db->query("SELECT
				  ...
				  a.id_div = '" .$divisi. "'
				  AND
				  a.id_grouplayanan = '" .$grouplayanan. "'")->result();
		
		if (!empty($cek)){   
		
		$status	= array(
			  'pesan'		=>'Gagal Save Layanan Sudah Ada. Thanks ...',
			  'status'	=> 0
			);
		...
		$insert = $this->db->insert("rs_grouplayanan_user",$dataDT);
	...
	// PROSES SAVE USER LAYANAN
	// -------------------------------------------------------------
	public function delete_userslayanan(){
		...
		$grouplayanan = $this->input->post('layanan');
		...
		$deleted = $this->db->delete('rs_grouplayanan_user', array('id_grouplayanan' => $grouplayanan, 'id_div' => $divisi)); 
